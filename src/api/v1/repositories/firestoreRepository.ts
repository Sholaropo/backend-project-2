...
/**
 * Queries documents in a collection that match a specific field value.
 *
 * @param collectionName - The name of the collection to query
 * @param fieldName - The name of the field to filter by
 * @param fieldValue - The value to match against
 * @returns Promise resolving to a QuerySnapshot containing matching documents
 * @throws RepositoryError if querying documents fails
 * 
 * @example
 * const snapshot = await queryDocumentsByField('employees', 'branchId', 'branch123');
 */
export const queryDocumentsByField = async (
    collectionName: string,
    fieldName: string,
    fieldValue: FirestoreDataTypes
): Promise<FirebaseFirestore.QuerySnapshot> => {
    try {
        ...
        throw new RepositoryError(
            `Failed to query documents in ${collectionName} where ${fieldName} = ${fieldValue}: ${getErrorMessage(
                error
            )}`,
            getErrorCode(error),
            getFirebaseErrorStatusCode(error)
